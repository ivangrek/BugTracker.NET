...
/// <summary>
///     Erweitert den System.Net.WebClient um Funktionen zum expliziten Ausführen von HTTP-GET- und HTTP-POST-Anfragen, bei
///     denen Cookies für eine Session bestehen bleiben.
///     http://www.mycsharp.de/wbb2/print.php?threadid=72666&page=1&sid=54b07d90ce6c6321ca74d93b15f55461
/// </summary>
// [System.Diagnostics.DebuggerStepThrough]
public class ExtendedWebClient : WebClient
{
    private static readonly RemoteCertificateValidationCallback ignoreInvalidCertificateValidationCallback =
        delegate { return true; }; // akzeptiert alle Zertifikate

    private readonly CookieContainer
        cookieContainer = new CookieContainer(); // um bei Login-Szenarien auch eingeloggt zu bleiben

    /// <summary>
    ///     Ruft einen Wert ab, der bestimmt, ob die Gültigkeit von Cookies immer auf Verzeichnisebene gesetzt wird, oder legt
    ///     diesen fest.
    /// </summary>
    public bool ForceApplyCookiesToDirectories { get; set; }

    /// <summary>
    ///     Ruft einen Wert ab, der bestimmt, ob auch vom Betriebssystem als unsicher betrachtete (SSL-)Zertifikate erlaubt
    ///     werden, oder legt diesen fest,
    /// </summary>
    public static bool IgnoreInvalidCertificates
    {
        get => ServicePointManager.ServerCertificateValidationCallback == ignoreInvalidCertificateValidationCallback;
        ...
        if (webRequest is HttpWebRequest)
            lock (this)
            {
                ...
                if (HttpWebRequestCreated != null)
                    HttpWebRequestCreated((HttpWebRequest) webRequest);
            }
    ...
    protected override WebResponse GetWebResponse(WebRequest request)
    {
        ...
        if (webResponse is HttpWebResponse)
            lock (this)
            {
                ...
                if (ForceApplyCookiesToDirectories)
    ...
    {
        return post(uri, createParamString(objectToDictionary(parameters)));
    }
    ...
    {
        return post(uri, createParamString(parameters));
    }
    ...
    {
        using (var webResponse = GetWebResponse(post(new Uri(uri), parameters)))
        {
            return ReadWebResponse(webResponse);
        }
    }
    ...
    private WebRequest post(Uri uri, string parameters)
    {
        ...
        using (var streamWriter = new StreamWriter(webRequest.GetRequestStream(), Encoding.Default))
        {
    ...
    {
        return get(uri, createParamString(objectToDictionary(parameters)));
    }
    ...
    {
        return get(uri, createParamString(parameters));
    }
    ...
    {
        using (var response = GetWebResponse(get(new Uri(uri), parameters)))
        {
    ...
    {
        var encoding = webResponse is HttpWebResponse
            ? Encoding.GetEncoding(((HttpWebResponse) webResponse).CharacterSet)
            : Encoding.Default;
    ...
    private static string createParamString(IDictionary<string, string> parameters)
    {
        ...
        foreach (var key in parameters.Keys)
            result.Add(HttpUtility.UrlEncode(key) + "=" + HttpUtility.UrlEncode(parameters[key]));

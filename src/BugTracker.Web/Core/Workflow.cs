...
// upgrade.
    ...
    public class Workflow
    {
            ...
            HtmlContainerControl custom_validation_err_msg)
        {
            ...
            var category = (DropDownList) find_control(page.Controls, "category");
            var priority = (DropDownList) find_control(page.Controls, "priority");
            ...
            Page page) // the options in the dropdown
        {
            ...
            var status = (DropDownList) find_control(page.Controls, "status");

            if (bug != null) // existing bug
            {
                // Get the bug's current status.
                // See "get_bug_datarow()" in bug.cs for the sql
                var status_name = (string) bug["status_name"];

...
namespace BugTracker.Web
{
    ...
    public partial class HgViewRevisions : Page
    {
        public int Bugid;
        public DataSet Ds;
...
case when hgap_action not like '%D%' and hgap_action not like 'A%' then
	'<a target=_blank href=HgDiff.aspx?revpathid=' + convert(varchar,hgap_id) + '>diff</a>'
	else
	''
end [view<br>diff],

case when hgap_action not like '%D%' then
'<a target=_blank href=HgLog.aspx?revpathid=' + convert(varchar,hgap_id) + '>history</a>'
	else
	''

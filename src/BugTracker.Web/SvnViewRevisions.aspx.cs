...
namespace BugTracker.Web
{
    ...
    public partial class SvnViewRevisions : Page
    {
        public int Bugid;
        public DataSet Ds;
...
case when svnap_action not like '%D%' and svnap_action not like 'A%' then
	'<a target=_blank href=SvnDiff.aspx?revpathid=' + convert(varchar,svnap_id) + '>diff</a>'
	else
	''
end [view<br>diff],

case when svnap_action not like '%D%' then
'<a target=_blank href=SvnLog.aspx?revpathid=' + convert(varchar,svnap_id) + '>history</a>'
	else
	''
